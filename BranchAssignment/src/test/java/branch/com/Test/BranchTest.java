...
import branch.common.BranchAssignment.Base;
import junit.framework.Assert;

static List<String> allNames = new ArrayList<>(), employeeNamesBranchWise = new ArrayList<>();
...
private void navigateToBranch(){
	...
	try{
		Assert.assertTrue(driver.getTitle().contains("Branch"));
...
public void verifyMemberCount(){
	...
	try{
		Assert.assertEquals(totalMembers, categoryMemberCount);
	}catch(AssertionError e){
...
public void verifyNames(){
	...
	try{
		...
		Assert.assertEquals(allNames, employeeNamesBranchWise);
	}catch(AssertionError e){
...
public void getNameDepartment(){
	...
	try{
		Assert.assertTrue(mapsAreEqual(allEmployeeNameDepartement, branchWiseEmployeeNameDepartment));
	}catch(AssertionError e){
